  ...
  return new Promise((fertig, fehlgeschlagen) => {
    ...
    raus.on('finish', () => {
      ...
      try {
        ...
        aufraeumen();
        // Der Systemfehler enthaelt den vollen Pfad. Er wird mitgegeben, aber
        // gesondert - der Aufrufer entscheidet, ob er nach draussen darf.
        const raus = new Error('Ablegen fehlgeschlagen.');
        raus.systemfehler = e && e.message;
        return fehlgeschlagen(raus);
      }
...
//
// ziel sagt, woran die Anzeige haengt: 'pi' kann HEVC schlicht nicht, ein
// Rechner meistens schon. Ein Hinweis, der auf einem Windows-Rechner "spielt
// ein Raspberry Pi nicht ab" meldet, verunsichert nur - der Clip laeuft ja.
function formatHinweis(datei, ziel) {
  ...
  const amPi = ziel !== 'rechner';
  const codec = videoCodec(datei);
  if (codec === 'h264') return null;

  if (codec === 'hevc') {
    if (!amPi) {
      return 'Dieser Clip ist in HEVC (H.265) aufgenommen - so nehmen iPhones standardmäßig auf. '
           + 'Die meisten Rechner spielen das ab. Falls er auf dem Bildschirm übersprungen wird: '
           + 'am Rechner unter "Videos - Clips prüfen & umwandeln" nach MP4 (H.264) umwandeln.';
    }
  ...
  if (codec === 'av1' || codec === 'vp9') {
    const name = codec === 'av1' ? 'AV1' : 'VP9';
    if (!amPi) {
      return 'Dieser Clip ist in ' + name + ' kodiert. Das braucht spürbar Rechenleistung; '
           + 'ruckelt es, hilft Umwandeln nach MP4 mit H.264.';
    }
    return 'Dieser Clip ist in ' + name + ' kodiert. Auf einem '
